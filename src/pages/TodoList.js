...
const TodoList = () => {
  ...
  const fetchTodos = async () => {
    try {
      const response = await axios.get('https://fh9d7gb671.execute-api.eu-north-1.amazonaws.com/todo-prod/todos', {
        params: {
  ...
  const addTodo = async () => {
    if (newTodo.trim() !== '') {
      try {
        await axios.post('https://fh9d7gb671.execute-api.eu-north-1.amazonaws.com/todo-prod/todo', { title: newTodo, completed: false, username: username });
        setNewTodo('');
        fetchTodos();
      } catch (error) {
  ...
  const deleteTodo = async (id) => {
    try {
      await axios.delete(`https://fh9d7gb671.execute-api.eu-north-1.amazonaws.com/todo-prod/todo`, { data: { id } });
      fetchTodos();
    } catch (error) {
  ...
  const toggleComplete = async (id, completed) => {
    try {
      await axios.patch(`https://fh9d7gb671.execute-api.eu-north-1.amazonaws.com/todo-prod/todo`, { id, updateKey: 'completed', updateValue: !completed });
      fetchTodos();
    } catch (error) {
  ...
  const saveEditing = async (id, text) => {
    try {
      await axios.patch(`https://fh9d7gb671.execute-api.eu-north-1.amazonaws.com/todo-prod/todo`, { id, updateKey: 'title', updateValue: text });
      setEditingId(null);
      fetchTodos();
    } catch (error) {
      console.error('Error updating todo:', error);
